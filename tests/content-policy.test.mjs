...
test("native OpenSAT exports convert locally without bundling upstream content", () => {
  ...
  assert.equal(record.content.conceptId, "algebra");
  assert.equal(record.content.representation, undefined);
  ...
  const [withoutSentinel] = parseOpenSatQuestions(
    JSON.stringify([
      {
        ...row,
        id: "original-demo-2",
        question: { ...row.question, paragraph: "null" },
      },
    ]),
  );
  assert.equal(withoutSentinel.content.prompt.startsWith("null"), false);
});
